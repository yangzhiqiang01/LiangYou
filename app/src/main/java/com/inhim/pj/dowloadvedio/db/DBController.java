package com.inhim.pj.dowloadvedio.db;

import android.content.Context;

import com.inhim.downloader.db.DownloadDBController;
...
import com.inhim.pj.dowloadvedio.domain.MyDownloadInfLocal;
import com.inhim.pj.dowloadvedio.domain.MyDownloadThreadInfoLocal;
...
public class DBController implements DownloadDBController {
  ...
  public List<DownloadInfo> findAllDownloading() {
    try {
      List<MyDownloadInfLocal> myDownloadInfLocals = myDownloadInfLocalDao.queryBuilder().where()
          .ne("status", DownloadInfo.STATUS_COMPLETED).query();
  ...
  public List<DownloadInfo> findAllDownloaded() {
    try {
      List<MyDownloadInfLocal> myDownloadInfLocals = myDownloadInfLocalDao.queryBuilder().where()
          .eq("status", DownloadInfo.STATUS_COMPLETED).query();
      return convertDownloadInfos(myDownloadInfLocals);
    } catch (SQLException e) {
      e.printStackTrace();
    }
  ...
  public void pauseAllDownloading() {
    ...
    try {
      UpdateBuilder<MyDownloadInfLocal, Integer> myDownloadInfLocalIntegerUpdateBuilder = myDownloadInfLocalDao
          .updateBuilder();
      myDownloadInfLocalIntegerUpdateBuilder.updateColumnValue("status", DownloadInfo.STATUS_PAUSED).where()
          .ne("status", DownloadInfo.STATUS_COMPLETED);
